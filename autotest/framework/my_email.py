...
date = time.strftime('%Y%m%d', time.localtime(time.time()))  # 获取本地时间 转换成日期
...
file = os.path.join(path, 'report/测试报告.html')
...
def mail():
    try:
        ...
        message['Subject'] = date + "测试报告.html"  # 邮件的主题，也可以说是标题

        # 邮件正文内容
        message.attach(MIMEText('附件为' + date + '测试报告.html', 'plain', 'utf-8'))
